...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (submissionForm) {
    submissionForm.addEventListener("submit", async (e) => {
      ...
      const hiddenContainers = submissionForm.querySelectorAll(
        ".hidden-event-container"
      );
      hiddenContainers.forEach((container) => {
        const sdgValues = Array.from(
          container.querySelectorAll(".event-sdg-value")
        ).map((inp) => inp.value);
        const eventData = {
      ...
      if (submissionData.events.length === 0) {
        ...
        const eventData = {
          ...
          eventDescription:
            inlineContainer.querySelector("#eventDesc")?.value || "",
          attendance: parseInt(
            inlineContainer.querySelector("#eventAttendees")?.value || 0
          ),
      ...
      try {
        ...
        const createResponse = await fetch(
          "../../php-server/routes/submissions.php",
          {
            method: "POST",
        ...
        const fileUploadResult = await uploadSubmissionFiles(
          submissionId,
          orgAcronym
        );
        ...
        if (!fileUploadResult.success && fileUploadResult.hasFiles) {
          throw new Error(
            "File upload failed: " + (fileUploadResult.error || "Unknown error")
          );
        }
