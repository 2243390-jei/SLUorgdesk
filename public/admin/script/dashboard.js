...
class DashboardAnalytics {
    ...
    async fetchData() {
        try {
            ...
            const orgsResponse = await fetch('../php-server/routes/organizations.php');
            if (!orgsResponse.ok) throw new Error('Failed to fetch organizations');
            const orgsResult = await orgsResponse.json();
            this.orgsData = orgsResult.success ? orgsResult.data : [];

            // Fetch users data
            const usersResponse = await fetch('../php-server/routes/users.php');
            if (!usersResponse.ok) throw new Error('Failed to fetch users');
            const usersResult = await usersResponse.json();
            this.usersData = usersResult.success ? usersResult.data : [];
